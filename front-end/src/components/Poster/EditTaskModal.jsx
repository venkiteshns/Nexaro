...
import { useUpdateTaskMutation } from '../../store/services/api';
...
const EditTaskModal = ({ task, onClose, onUpdated }) => {
    ...
    const [urgencyLevel, setUrgencyLevel] = useState(task.urgencyLevel || 'flexible');
    ...
    const handleRemoveRetained = (publicId) => {
        setRetainedImages((prev) => prev.filter((img) => img.public_id !== publicId));
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await updateTask({
                taskId: task._id,
                formValues: { title, description, category, deadline, urgencyLevel, amount, newPhotos },
                retainedImages,
            }).unwrap();
            ...
            showError(err?.data?.message || 'Failed to update task. Please try again.');
        }
    };
    ...
    return (
        ...
        >
            ...
            >
                {/* Header */}
                <div className="h-1.5 w-full bg-gradient-to-r from-[#0A6E5C] to-emerald-400 shrink-0" />
                ...
                <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
                    <form id="edit-task-form" onSubmit={handleSubmit} noValidate>
                        ...
                        {/* Title */}
                        <div className="mb-4">
                            <label className="block text-sm font-medium text-gray-600 mb-1.5">Task Title</label>
                            <input
                                type="text"
                                value={title}
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                            <div>
                                <label className="block text-sm font-medium text-gray-600 mb-1.5">Preferred Deadline</label>
                                <input
                                    type="date"
                                    value={deadline}
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-600 mb-1.5">Urgency Level</label>
                        ...
                        <div className="border border-gray-100 rounded-2xl p-4 bg-gray-50/50">
                            <div className="flex items-center gap-2 mb-3">
                                ...
                                <h3 className="text-sm font-semibold text-gray-800">Task Photos</h3>
                                <span className="ml-auto text-xs text-gray-400">{totalImageCount}/5</span>
                            </div>
                            ...
                            <div className="flex gap-3 flex-wrap">
                                ...
                                {totalImageCount < 5 && (
                                    <label
                                        className="w-24 h-24 border-2 border-dashed border-gray-300 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:border-[#0A6E5C] hover:bg-emerald-50 transition-all group"
                                    >
                                        <Upload size={18} className="text-gray-400 group-hover:text-[#0A6E5C] mb-1 transition-colors" />
                                        <span className="text-[10px] text-gray-400 group-hover:text-[#0A6E5C] transition-colors">Add Photo</span>
                                        <input
                                            type="file"
                                            accept="image/*"
                                            multiple
                                ...
                                {retainedImages.map((img) => (
                                    <div key={img.public_id} className="w-24 h-24 rounded-xl overflow-hidden border border-gray-200 relative shrink-0">
                                        ...
                                        <div className="absolute bottom-1 left-1 bg-emerald-500 text-white text-[9px] px-1.5 py-0.5 rounded-md font-semibold">Saved</div>
                                    </div>
                                ))}

                                {/* New local images */}
                                {newPhotos.map((file, i) => (
                                    <div key={`new-${i}`} className="w-24 h-24 rounded-xl overflow-hidden border border-emerald-300 relative shrink-0">
                                        ...
                                        <div className="absolute bottom-1 left-1 bg-blue-500 text-white text-[9px] px-1.5 py-0.5 rounded-md font-semibold">New</div>
                                    </div>
                                ))}

                                {/* Empty slots */}
                                {Array.from({ length: Math.max(0, 4 - retainedImages.length - newPhotos.length) }).map((_, i) => (
                                    <div key={`empty-${i}`} className="w-24 h-24 border border-gray-100 rounded-xl bg-gray-50 flex items-center justify-center shrink-0">
                            ...
                            <p className="text-[11px] text-gray-400 mt-2">Add up to 5 photos. Tap ✕ to remove. "Saved" = existing images already on the task.</p>
                        </div>
